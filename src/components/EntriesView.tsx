...
import { getCollections, deleteEntry, type DataCollection, type DataEntry } from "@/lib/store";
...
export default function EntriesView({ refreshKey }: Props) {
  ...
  const formatDate = (d: string) =>
    new Date(d).toLocaleDateString("de-DE", { weekday: "short", day: "numeric", month: "short", year: "numeric" });
  ...
  return (
    <div className="px-5 pt-3 pb-24">
      <div className="flex items-center justify-between mb-4 animate-fade-up">
        <h1 className="text-2xl text-display text-foreground">Einträge</h1>
      ...
      {showSearch && (
        <input
          ...
          placeholder="Suchen..."
          className="w-full h-10 px-4 rounded-xl bg-muted text-foreground text-sm mb-4 placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring animate-fade-up"
          autoFocus
        />
      )}
        ...
        {collections.map(c => (
          <button
            ...
            className={`flex-shrink-0 px-3.5 py-1.5 rounded-xl text-xs font-medium transition-all active:scale-95 ${
              activeId === c.id ? "text-primary-foreground card-shadow" : "bg-muted text-muted-foreground"
            }`}
      ...
      <div className="space-y-2">
        {filtered.length === 0 && (
          <p className="text-center text-muted-foreground text-sm py-10">Keine Einträge gefunden.</p>
        )}
        {filtered.map((entry, i) => (
          <div
            key={entry.id}
            className="bg-card rounded-xl p-3.5 card-shadow flex items-center justify-between animate-fade-up"
          ...
          >
            ...
            <button
              ...
              className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground hover:text-destructive active:scale-90 transition-all"
            >
